...
table(foo$isBigCount)
table(foo$nomFreq)

bar <- df.alltags %>% 
  filter(isBigCount == "Not Big Count")
table(bar$tagDeployStart)
table(df.alltags$tagDeployStart)
table(june_30_only$tagDeployStart, useNA = "ifany")
```
